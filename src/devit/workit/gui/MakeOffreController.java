...
public class MakeOffreController implements Initializable {
 ...
 public void show() throws FileNotFoundException {
        ...
        if (tf.getText().equals("")) {
                ...
                String author_name = SessionWorkit.getPrenom();
                int idrec = SessionWorkit.getId();
        ...
        int nb1 = 0;
        nb1 = pos.returnRec(SessionWorkit.getId());
        Image image = new Image("/images/check.png");
        int nb5 = 0;
        nb5 = pos.returnvalid(SessionWorkit.getId(),se.getId());
        if (nb5 == 1 ) {
         ...
         int nb = 0;
            ...
            ligne.getChildren().addAll(col, coach,tvalid,tpost, hb);
             ...
             tvalid.setOnMouseClicked(e -> {
                     if (pos.returnRec(SessionWorkit.getId()) == SessionWorkit.getId()) {
            boolean test = false ;
                         try {
                             test = pos.Supprimer(SessionWorkit.getId());
                             acc();
            ...
            p.setRecruteur_id(SessionWorkit.getId());
            boolean test;
                         try {
                             pos.addPost(p);
                         } catch (SQLException ex) {
            ...
            /* String Code = null;
                        ...
                        // System.out.println(Code);
                         try {
                             mailer.sendMail("mohamedali.benchikha@esprit.tn");
                         } catch (Exception ex) {
                             Logger.getLogger(MakeOffreController.class.getName()).log(Level.SEVERE, null, ex);
                         }
                         try {
